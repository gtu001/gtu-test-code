...
import gtu.regex.tag.TagMatcher2;
...
public class RegexDirReplacer extends javax.swing.JFrame {
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        }
                        {
                            ...
                            {
                                lblNewLabel_1 = new JLabel("offset");
                                p1.add(lblNewLabel_1);
                            }
                            {
                                startTagOffsetText = new JTextField();
                                p1.add(startTagOffsetText);
                                startTagOffsetText.setColumns(10);
                                startTagOffsetText.getDocument().addDocumentListener(JCommonUtil.getDocumentListener(new HandleDocumentEvent() {
                                    @Override
                                    public void process(DocumentEvent event) {
                                        startTagOffsetText.setText("" + getInt(startTagOffsetText.getText()));
                                    }
                                }));
                            }
                        }
                        {
                            ...
                            {
                                endTagOffsetText = new JTextField();
                                endTagOffsetText.setColumns(10);
                                p1.add(endTagOffsetText);
                                endTagOffsetText.getDocument().addDocumentListener(JCommonUtil.getDocumentListener(new HandleDocumentEvent() {
                                    @Override
                                    public void process(DocumentEvent event) {
                                        endTagOffsetText.setText("" + getInt(endTagOffsetText.getText()));
                                    }
                                }));
                            }
                        }
    ...
    private JTextField startTagOffsetText;
    private JTextField endTagOffsetText;
    ...
    private Integer getInt(String val) {
        try {
            return Integer.parseInt(StringUtils.trimToEmpty(val));
        } catch (Exception ex) {
            return 0;
        }
    }
    ...
    String replacer(String fromPattern, String toFormat, String replaceText, int waitSecond, AtomicReference<Throwable> result) {
        ...
        new Thread(new Runnable() {
            ...
            private String tagProcess(String replaceText) {
                ...
                int startTagOffset = getInt(startTagOffsetText.getText());
                int endTagOffset = getInt(endTagOffsetText.getText());
                String content = replaceText.toString();

                TagMatcher2 tag = new TagMatcher2(startTag, endTag, startPtnStr, endPtnStr, startTagOffset, endTagOffset, content);
