...
public class ReadXmlGenerateJavaBean {
    ...
    public static void main(String[] args) throws DocumentException, IOException, SecurityException, NoSuchFieldException {
        ...
        showElementTree_toJavaBean(doc.getRootElement(), javaBeanMap, false);
     ...
     * 以xml產出javaBean
     */
    private static void showElementTree_toJavaBean(Element element, Map<String, String> javaBeans, boolean isXmlDbType) {
        List<Element> needTraceList = new ArrayList<Element>();
        List<Element> elist = element.elements();
        StringBuilder sb = new StringBuilder();
        sb.append(fetchAnnotation(element) + "\n");
        sb.append("private static class " + StringUtils.capitalize(getElementParam(element.getName(), isXmlDbType)) + " { \n");
        ...
        for (Element ie : needTraceList) {
            showElementTree_toJavaBean(ie, javaBeans, isXmlDbType);
        }
    }
