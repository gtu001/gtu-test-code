...
Dates.getI18nMonth = (function (month) {
    var monthNum = parseInt(month,10);
    if(typeof i18nMessage != 'undefined' && i18nMessage instanceof Function) {
        if (monthNum === 1) {
            return i18nMessage('1月', 'laydate-january')
        } else if (monthNum === 2) {
            return i18nMessage('2月', 'laydate-february')
        } else if (monthNum === 3) {
            return i18nMessage('3月', 'laydate-march')
        } else if (monthNum === 4) {
            return i18nMessage('4月', 'laydate-april')
        } else if (monthNum === 5) {
            return i18nMessage('5月', 'laydate-may')
        } else if (monthNum === 6) {
            return i18nMessage('6月', 'laydate-june')
        } else if (monthNum === 7) {
            return i18nMessage('7月', 'laydate-july')
        } else if (monthNum === 8) {
            return i18nMessage('8月', 'laydate-august')
        } else if (monthNum === 9) {
            return i18nMessage('9月', 'laydate-september')
        } else if (monthNum === 10) {
            return i18nMessage('10月', 'laydate-october')
        } else if (monthNum === 11) {
            return i18nMessage('11月', 'laydate-november')
        } else if (monthNum === 12) {
            return i18nMessage('12月', 'laydate-december')
        }
    } else {
        return month+"月";
    }
...
Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            Dates.elem[as.elemv] = '';
            if(typeof i18nMessage != 'undefined' && i18nMessage instanceof Function){
                Dates.msg(i18nMessage('日期不符合格式，请重新选择。','laydate-001'));
            } else {
            ...
            Dates.elem[as.elemv] = '';
            if(typeof i18nMessage != 'undefined' && i18nMessage instanceof Function) {
                Dates.msg(i18nMessage('日期不在有效期内，请重新选择。','laydate-002'));
            } else {
...
Dates.viewDate = function(Y, M, D){
    ...
    if(typeof i18nMessage != 'undefined' && i18nMessage instanceof Function) {
        if(sessionStorage.getItem("$language") === "en"){
            ...
            as.year.value = Dates.ymd[0] + i18nMessage('年','laydate-year');
        }
        var monthNum = Dates.digit(Dates.ymd[1] + 1);
        as.month.value = Dates.getI18nMonth(monthNum);
    } else {
...
Dates.viewtb = (function(){
    ...
    // if(typeof i18nMessage != 'undefined' && i18nMessage instanceof Function){ //初始化时来不及获取i18n信息
    //     weeks = [ i18nMessage('日','laydate-sunday'),  i18nMessage('一','laydate-monday'),  i18nMessage('二','laydate-tuesday'),  i18nMessage( '三','laydate-wenseday'),  i18nMessage('四','laydate-thursday'),  i18nMessage( '五','laydate-friday'),  i18nMessage('六','laydate-saturday')];
    // } else {
        if(sessionStorage.getItem("$language") === "en"){
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        var times ="时间";
        if(typeof i18nMessage != 'undefined' && i18nMessage instanceof Function){
            clear = i18nMessage("清空",'laydate-clear');
            ok = i18nMessage("确认",'laydate-confirm');
            today = i18nMessage("今天",'laydate-today');
            times = i18nMessage("时间",'laydate-times');
        }
          ...
          +'<div class="laydate_ym laydate_m" id="laydate_MM">'
            ...
            +'<div class="laydate_yms" id="laydate_ms">'+ function(){
                ...
                Dates.each(new Array(12), function(i){
                    str += '<span m="'+ i +'">'+ Dates.getI18nMonth(Dates.digit(i+1)) +'</span>';
                });
                return str;
            }() +'</div>'
          +'</div>'
...
Dates.creation = function(ymd, hide){
  ...
  //bug:71 modify begin by zhaoyanyang 2018/05/02  获取input对象  默认触发投被保人填写出生日期后改变年龄
    var objName = Dates.options.elem; 
	var obj = $(objName);
	 if(obj){
 		obj.focus();         
 		obj.blur();
...
Dates.events = function(){
    ...
    Dates.on(as.otoday, 'click', function(){
    	var objName = Dates.options.elem; // wjw 2017年5月19日16:43:09  获取input对象  默认触发投被保人填写出生日期后改变年龄
    	var obj = $(objName);
      ...
      //bug:71 modify end by zhaoyanyang 2018/05/02  获取input对象  默认触发投被保人填写出生日期后改变年龄
    });
