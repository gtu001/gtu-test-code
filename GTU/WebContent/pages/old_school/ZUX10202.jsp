...
function ZUX10202(){
	function SerialNoList() {
		...
		this.initDetail = function(serNoList, modNo, serNo){
			...
			/*
				...
				*/
				var span = new Element('span', {"class":"tabs_menu","jsonObj":Object.toJSON(dtlArry[ii])}).update(" " + (ii + 1) + " ");
				serNoList.appendChild(span);
	...
	function ApplyMapX112(reqMap, mapX112, x125Map) {
		...
		/*
			...
			*/
			...
			var def = this.COL_PROPERTIES[name];
			var input = new Element('input', {"nullable":def['NULLABLE'],"COL_TYPE":def['COL_TYPE'],"maxlength":def['COL_LENGTH'],"name":namePrefix + "_" + name});
			if(namePrefix == "Q"){
				input.value = this.Qx125Map[name] || "";
			}else if(namePrefix == "U"){
				input.value = this.Ux125Map[name] || "";
			...
			if("DATE" == def['COL_TYPE']) { 
				...
				*/
				...
				localeDisplay.registerToDate(namePrefix + "_" + name);
			}
			return input;
		};
		...
		/*
			...
			btn.appendChild(document.createTextNode("查詢"));
			btn.setAttribute("class", "button");
			*/
			var btn = new Element('button', {"id":"BTN_query","type":"button","value":"查詢","class":"button"}).update("查詢");
		...
		/*
			...
			btn.setAttribute("class", "button");
			*/
			var btn = new Element('button', {"id":id,"type":"button","value":value,"class":"button"}).update(value);
			btn.addEventListener("click", function(e){
				func();
			});
			return btn;
		...
		this.emptyTbody = function(){
			$("Q_COLs_tbody").emptyChildren();
			$("U_COLs_tbody").emptyChildren();
		};
		...
		this.isBtn_showControl = function() {
			if(!this.reqMap.EXEC_NO || this.reqMap.EXEC_NO == ''){
				$("BTN_doSaveValues_Input").show(true);
				$("BTN_doSaveValues_Update").show(false);
				$("BTN_doSaveValues_Delete").show(false);
			}else{
				$("BTN_doSaveValues_Input").show(false);
				$("BTN_doSaveValues_Update").show(true);
				$("BTN_doSaveValues_Delete").show(true);
			}
		};
		
		this.applyUCols = function(){
			var U_COLsTbody = $("U_COLs_tbody");
	...
	function ValTableControl() {
		...
		this.creatSQL_LINK = function(dataList) {
			...
			$('SQL_LINK').emptyChildren();
			
			var sqlLink = $("SQL_LINK");
		...
		this.createUPDSQLTable = function(rtnMap,TABLE_PROPERTIES_MAP) {
			var QueryTable = $('Q_Table');
			var SQLTable = $('U_Table');
		...
		this.createINSSQLTable = function(rtnMap,TABLE_PROPERTIES_MAP){
			...
			$('I_TableName').innerHTML = TABLE_PROPERTIES_MAP['TABLE_REMARKS']+'</br>('+rtnMap.TBL_NAME+')';
	...
	this.initApp = function() {
		<cathay:LocaleDisplay sysCode="ZU" dateFields="" dateymFields="" dateyFields="" numberFields="" var="localeDisplay"/>
		
		mZUX10202.commonUsageFunc();
...
<input type="hidden" id="FORM_NO" name="FORM_NO" value="${mapX120.FORM_NO}" title="FORM_NO" /> 
<input type="hidden" id="MOD_NO" name="MOD_NO" value="${mapX120.MOD_NO}" title="MOD_NO" /> 
<input type="hidden" id="EXEC_NO" name="EXEC_NO" value="" title="EXEC_NO" /> 
<input type="hidden" id="SER_NO" name="SER_NO" value="" title="SER_NO" /> 
<%--↓↓↓↓↓↓↓↓ 資料修正作業  ↓↓↓↓↓↓↓↓--%>
	...
	<thead>
		<c:forEach var="mapX111" items="${listX111}" varStatus="idx">
			<tr>
				...
				<a href="javascript:mZUX10202.actions.doQuery0022('${mapX111.FORM_NO}', 
	...
	<thead>
		...
		<tr>
			...
			<TABLE width=100% border=0 cellpadding="0" cellspacing="1"
				...
				<tr>
					<td width="100%" class="tbBlue" style="white-space: nowrap"
						align="center" colspan="10">設定檔</td>
				</tr>
				<tr>
					<td width="100%" class="tbBlue" style="white-space: nowrap"
						align="center" colspan="10">TABLE</td>
				</tr>
				<tr>
					<td width="25%" class="tbBlue" style="white-space: nowrap"
