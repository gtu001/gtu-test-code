...
public class HikariConfig implements HikariConfigMXBean {
     ...
     * <code>propertyFileName</code> will first be treated as a path in the
     * file-system, and if that fails the
     ...
     * Using the JDBC4 <code>Connection.isValid()</code> method to test
     * connection validity can be more efficient on some databases and is
     * recommended. See {@link HikariConfig#setJdbc4ConnectionTest(boolean)}.
     *
    ...
    /**
     * Get the SQL string that will be executed on all new connections when they
     * are created, before they are added to the pool.
     *
    ...
    /**
     * Set the SQL string that will be executed on all new connections when they
     * are created, before they are added to the pool. If this query fails, it
     * will be treated as a failed connection attempt.
     *
    ...
    /**
     * Get the {@link DataSource} that has been explicitly specified to be
     * wrapped by the pool.
     *
    ...
    /**
     * Set a {@link DataSource} for the pool to explicitly wrap. This setter is
     * not available through property file based initialization.
     *
     * @param dataSource
        ...
        try {
            ...
            if (threadContextClassLoader != null && threadContextClassLoader != this.getClass().getClassLoader()) {
                try {
                    ...
                    LOGGER.error("Failed to load class of driverClassName {} in either of HikariConfig class classloader {} or Thread context classloader {}", driverClassName,
                            this.getClass().getClassLoader(), threadContextClassLoader);
                }
            } else {
    ...
    /**
     * Get the pool initialization failure timeout. See
     * {@code #setInitializationFailTimeout(long)} for details.
     *
     ...
     * {@link HikariConfig}, or when {@link HikariDataSource} is constructed
     * using the no-arg constructor and {@link HikariDataSource#getConnection()}
     * is called.
     * <ul>
     * <li>Any value greater than zero will be treated as a timeout for pool
     * initialization. The calling thread will be blocked from continuing until
     * a successful connection to the database, or until the timeout is reached.
     * If the timeout is reached, then a {@code PoolInitializationException}
     * will be thrown.</li>
     * <li>A value of zero will <i>not</i> prevent the pool from starting in the
     * case that a connection cannot be obtained. However, upon start the pool
     * will attempt to obtain a connection and validate that the
     * {@code connectionTestQuery} and {@code connectionInitSql} are valid. If
     * those validations fail, an exception will be thrown. If a connection
     * cannot be obtained, the validation is skipped and the the pool will start
     ...
     * exceptions.</li>
     * <li>A value less than zero will <i>not</i> bypass any connection attempt
     * and validation during startup, and therefore the pool will start
     * immediately. The pool will continue to try to obtain connections in the
     * background. This can mean that callers to
     * {@code DataSource#getConnection()} may encounter exceptions.</li>
     * </ul>
     * Note that if this timeout value is greater than or equal to zero (0), and
     * therefore an initial connection validation is performed, this timeout
     * does not override the {@code connectionTimeout} or
     * {@code validationTimeout}; they will be honored before this timeout is
     * applied. The default value is one millisecond.
     *
     * @param initializationFailTimeout
     *            the number of milliseconds before the pool initialization
     *            fails, or 0 to validate connection setup but continue with
     *            pool start, or less than zero to skip all initialization
     *            checks and start the pool without delay.
     */
     ...
     *
     * @param failFast
     *            true if the pool should fail if the minimum connections cannot
     *            be created
     ...
     *            the Codahale MetricRegistry to set
     */
    public void setMetricRegistry(Object metricRegistry) {
        ...
        if (metricRegistry != null) {
            ...
            if (!(metricRegistry.getClass().getSimpleName().equals("MetricRegistry"))) {
     ...
     *            the Codahale HealthCheckRegistry to set
     */
    public void setHealthCheckRegistry(Object healthCheckRegistry) {
        if (healthCheckRegistry != null) {
            ...
            if (!(healthCheckRegistry.getClass().getSimpleName().equals("HealthCheckRegistry"))) {
    ...
    /**
     * Get the default password to use for DataSource.getConnection(username,
     * password) calls.
     * 
     * @return the password
     */
    ...
    /**
     * Set the default password to use for DataSource.getConnection(username,
     * password) calls.
     * 
     * @param password
     *            the password
    ...
    /**
     * Get the default username used for DataSource.getConnection(username,
     * password) calls.
     *
     * @return the username
     */
    ...
    /**
     * Set the default username used for DataSource.getConnection(username,
     * password) calls.
     *
     * @param username
     *            the username
     ...
     *
     * @return the thread factory (may be null, in which case the default thread
     *         factory is used)
     */
     ...
     *
     * @param threadFactory
     *            the thread factory (setting to null causes the default thread
     *            factory to be used)
     */
    ...
    public void validate() {
        ...
        jdbcUrl = "jdbc:oracle:thin:@10.1.112.19:1521:LISDEV";
        this.setUsername("LIS");
        this.setPassword("lis");
        // ======================xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        ...
        if (dataSource != null) {
            ...
            if (driverClassName != null) {
                ...
                // NOTE: This exception text is referenced by a Spring Boot
                // FailureAnalyzer, it should not be
        ...
        for (String prop : propertyNames) {
            try {
                ...
                if ("initializationFailTimeout".equals(prop) && initializationFailTimeout <= 0) {
                    ...
                    value = "\"" + value + "\""; // quote to see lead/trailing
                                                 // spaces is any
                } else if (value == null) {
                    value = "none";
                }
    ...
    private int generatePoolNumber() {
        // Pool number is global to the VM to avoid overlapping pool numbers in
        // classloader scoped environments
