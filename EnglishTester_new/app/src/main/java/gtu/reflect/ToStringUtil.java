            ...
            public String apply(Object obj, Class<?> clz) {
                ...
                for (Field f : clz.getFields()) {
                    ...
                    if (Modifier.isStatic(modifier) && Modifier.isFinal(modifier)) {
                        try {
                            value = f.get(clz);
                            sbappendTabLine(StringUtils.rightPad("- [" + getModifier(f) + "] " + f.getType().getSimpleName() + " " + f.getName() + " = " + value, 70) //
                                    + declaredClass(f.getDeclaringClass()), sb);
                        } catch (Exception e) {
                        }
                    } else {
                        sbappendTabLine(StringUtils.rightPad("- [" + getModifier(f) + "] " + f.getType().getSimpleName() + " " + f.getName(), 70) //
                                + declaredClass(f.getDeclaringClass()), sb);
                    }
                }
                sb.append("  (method)" + CHANGE_LINE);
                ...
                for (Method m : clz.getMethods()) {
                    ...
                    sbappendTabLine(StringUtils.rightPad("- [" + getModifier(m) + "] " + m.getReturnType().getSimpleName() + " " + m.getName() + " (" + cb + ")", 70)//
