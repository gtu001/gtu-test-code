...
def writeContentCSV(yyy, lst):
    with open(fileUtil.getDesktopDir() + yyy + "_4_鄉鎮.csv", 'w', newline='', encoding='utf-8') as csvfile:
...
def getSheet(filePath):
    ...
    return wb.get_sheet_by_name("age_type=5")
...
def main(filePath, yyy):
    ...
    regionMap = getRegionDefMap(sheet, -2)
    if len(regionMap) != 0:
        global rowIdMapping
    ...
    for i, row in enumerate(sheet, 1):
        for j, cell in enumerate(row, 0):
            newVal = CellDef(cell.value, i, j, defMap, "age_type=5", 0)
            lst.append(newVal)
