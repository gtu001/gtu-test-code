...
import com.example.gtu001.qrcodemaker.MainActivity;
 ...
 * @author gtu001_5F
 */
public class UrlPlayerAssistService extends Service {

    private static final String TAG = UrlPlayerAssistService.class.getSimpleName();
    ...
    public void onCreate() {
        ...
        //啟動守護線程
        assistThread = UrlPlayerAssistService.createAssistThread(this, UrlPlayerService.class);
    }
    ...
    public static class AssistThreadClass extends Thread {
        ...
        @Override
        public void run() {
            while (UrlPlayerServiceHolderBroadcastReceiver.isFloatViewServiceEnable()) {
    ...
    public static int onStartCommandForOrignService(Intent intent, int flags, int startId, Service service) {
        ...
        builder.setSmallIcon(R.drawable.qr_code_icon);
        ...
        // 自定义打开的界面
        Intent resultIntent = new Intent(context, UrlPlayerService.class);
...
//        notificationManager.notify(pId, notification);
        ...
        //启动辅助Service
        Intent intent3 = new Intent(service, UrlPlayerAssistService.class);
        service.startService(intent3);

        //註冊廣播器
        IntentFilter filter = new IntentFilter(Intent.ACTION_TIME_TICK);
        context.registerReceiver(new UrlPlayerServiceHolderBroadcastReceiver(), filter);
